...
class Vector(tuple):
    ...
    def __init__(self, *V):
        '''The init method, we initialize the coordinates of a vector.
        You can initialize a vector for example: V = Vector() or
        V = Vector(a,b) or V = Vector(v1, v2, ..., vn)'''
        if len(V) == 0:
            self.V = (0, 0)
        elif len(V) == 1:
            raise ValueError('Sorry, a vector almost must have 2 coordinates.')
        else:
            self.V = V
    ...
    def __len__(self):
        '''Find out the dimmension of a vector V'''
        return len(self.V)

    def __getitem__(self, component):
        '''It gives the ith-component of a vector V'''
        return self.V[component]
